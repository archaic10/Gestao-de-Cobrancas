...
public class PagamentoDAO {
    ...
    public ArrayList<Pagamento> obterFaturamento(Date dataIni, Date dataF) throws SQLException, ParseException, Exception{
       ...
       String data_fim = formato.format(dataF);    
         String query = "SELECT SUM(valor_pago) AS faturamento  FROM pagamento WHERE periodo BETWEEN  '"+data_inicio+"' AND '"+data_fim+"' ";
        ...
        while (rs.next()) {
            ...
            double valor_pago = Double.parseDouble(rs.getString(1));            
            
            listaPagamento.add(new Pagamento(valor_pago));
        }
            return listaPagamento;
        } catch (SQLException sqle) {
            throw new Exception(sqle);
